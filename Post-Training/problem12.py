...
pairs = [
    ...
    {"positive": "Your performance is good",
     "negative": "Your performance is bad"},
...
def encode_batch_left_pad(
    texts, tokenizer, max_length=None, pad_token_id=50256
):
...
if __name__ == "__main__":
    ...
    for epoch in range(10):
        ...
        for elem in loader:
            optimizer.zero_grad()
            pos = torch.asarray(encode_batch_left_pad(
                elem['positive'], tokenizer,
                pad_token_id=tokenizer.vocab['<pad>']
            ))
            neg = torch.asarray(encode_batch_left_pad(
                elem['negative'], tokenizer,
                pad_token_id=tokenizer.vocab['<pad>']
            ))
