...
def build_tokenizer_and_reward_model():
    ...
    for _ in range(5):  # a few epochs is enough for demo
        ...
        for i in range(0, len(pairs), chunk):
            ...
            optimizer.zero_grad()
            pos = torch.asarray(encode_batch_left_pad(pos_texts, tokenizer, pad_token_id=tokenizer.vocab['<pad>']))
            neg = torch.asarray(encode_batch_left_pad(neg_texts, tokenizer, pad_token_id=tokenizer.vocab['<pad>']))
...
def rlhf_demo():
    ...
    for step in range(500):
        ...
        advantages = rewards[:, None] - kl_coef * kl
        ...
        if (step + 1) % 10 == 0:
            ...
            print(decoded)
            print(f"Step {step+1}: loss={ppo_loss.item():.4f}, reward={avg_reward:.4f}")
