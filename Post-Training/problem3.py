...
class Attention(nn.Module):
    ...
    def attention(self, x):
        ...
        k = self.k_proj(x)
        v = self.v_proj(x)
        q_headed = q.view(batch_size, length, self.head,
                          dim // self.head)
        k_headed = k.view(batch_size, length, self.head,
                          dim // self.head)
        v_headed = v.view(batch_size, length, self.head,
                          dim // self.head)
        pre_attention = torch.einsum(
            'blhd,bmhd->bhlm', q_headed, k_headed
        ) / sqrt(self.dim // self.head)
...
class Transformer(nn.Module):
    ...
    def __init__(self, vocab: int, dim: int, head: int, layers: int):
        super().__init__()
        self.vocab = vocab
    ...
    def forward(self, x: torch.Tensor):
        ...
        embeded_inputs = (
            embeded_inputs + self.pos_embed[:, :embeded_inputs.size(1)]
        )
        x = embeded_inputs
