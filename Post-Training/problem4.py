...
class Attention(nn.Module):
    ...
    def __init__(self, dim: int, head: int):
        ...
        inv_freq = (
            1.0 / (10000 ** (torch.arange(0, self.head_dim, 2).float()
                             / self.head_dim))
        )
    ...
    def _apply_rope(self, x: torch.Tensor):
        ...
        # Broadcast cos/sin: (seq_len, head_dim//2) -> (1, 1, seq_len,
        # head_dim//2)
    ...
    def attention(self, x):
        ...
        # Reshape to (batch, length, num_heads, head_dim) then transpose to
        # (batch, num_heads, length, head_dim)
        q_headed = q.view(
            batch_size, length, self.head, self.head_dim
        ).transpose(1, 2)
        k_headed = k.view(
            batch_size, length, self.head, self.head_dim
        ).transpose(1, 2)
        ...
        pre_attention = (
            torch.einsum('blhd,bmhd->bhlm', q_headed, k_headed)
            / sqrt(self.head_dim)
        )
        mask = torch.triu(
            torch.ones((length, length), device=x.device), diagonal=1
        ).bool()
...
class Transformer(nn.Module):
    ...
    def __init__(
        self, vocab: int, dim: int, head: int, layers: int,
        max_length: int = 20
    ):
        super().__init__()
        self.vocab = vocab
        ...
        self.attention_layers = nn.ModuleList(
            [Attention(dim, head) for _ in range(layers)]
        )
    ...
    def cutoff(self, t: torch.Tensor):
        ...
        # index of first zero per row
        first_zero_idx = torch.min(indices, dim=1).values

        # Output as list of lists (ragged)
        result = [
            (t[i, :first_zero_idx[i]].tolist() if first_zero_idx[i] < length
             else t[i].tolist())
...
if __name__ == '__main__':
    ...
    vocab = 500
    model = Transformer(
        vocab=vocab, dim=hidden_dim, head=2, layers=5
    )
    batch_size = 16
    length = 12
    steps = 1000
    ...
    for step in range(steps):
        ...
        targets = torch.cat([
            inputs[:, 1:],
            torch.ones((batch_size, 1), dtype=torch.int32) * model.eos
        ], -1)
        output = model(inputs)
        loss = loss_func(output.view((-1, output.shape[-1])),
                         targets.view(-1))
        loss.backward()
        optimizer.step()
