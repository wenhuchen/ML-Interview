...
class MultiHeadAttention(nn.Module):
	def __init__(self, emb_size: int, num_head: int, sliding_window: int):
		super().__init__()
	...
	def forward(self, q: torch.Tensor, cache_key: torch.Tensor = None, cache_value: torch.Tensor = None):
		...
		if cache_key is not None and cache_value is not None:
			...
			headed_key = headed_key[:, -self.sliding_window:, :, :]
			headed_value = headed_value[:, -self.sliding_window:, :, :]

			assert headed_key.shape[1] > q.shape[1], 'sliding window should be longer than the query!'

			mask = torch.cat([
				torch.zeros(q.shape[1], headed_key.shape[1] - q.shape[1]), 
				torch.triu(torch.ones(q.shape[1], q.shape[1]), diagonal=1)
			], -1)

			q_pos = torch.arange(0, mask.shape[0])
			k_pos = torch.arange(0, mask.shape[1])

			diff = q_pos[:, None] - k_pos[None, :]
			sliding_mask = (diff > 0)
			mask = mask.to(torch.bool) | sliding_mask
		else:
			...
			diff = q_pos[:, None] - k_pos[None, :]
			sliding_mask = (diff > self.sliding_window)
			mask = mask.to(torch.bool) | sliding_mask
