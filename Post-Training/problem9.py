...
def compute_neg_logp(model, rollouts, target_length):
    ...
    loss = torch.gather(
        neg_log_probs, dim=2, index=targets.unsqueeze(-1)
    ).squeeze(-1)
...
def evaluate(step: int, config: dict, model: nn.Module):
    ...
    print(
        f'step: {step}, success: {success/total:.3f}, '
        f'close: {close_matches/total:.3f}'
    )
    print('===============================================')
...
if __name__ == '__main__':
    ...
    for step in range(0, config['steps']):
        optimizer.zero_grad()
        inputs, targets = train_data_generator(config)
        unnormalized_rewards, _, rl_loss = rl_step(
            config, model, inputs, targets
        )
        rl_loss.backward()
        optimizer.step()
        print(
            f'step: {step}', rl_loss.item(), 'reward: ',
            unnormalized_rewards.mean().item()
        )
